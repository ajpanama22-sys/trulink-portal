...
export default function VendorPortalLicitaciones() {
  ...
  const enviarOferta = async () => {
    ...
    if (form.precio_unitario <= 0 || form.lead_time_dias <= 0) {
      return alert("Precio unitario y lead time son obligatorios.");
    }
    setGuardando(true);
    try {
      ...
      alert("Oferta enviada. Podés editarla mientras la licitación siga abierta.");
    ...
    <div style={{ backgroundColor: theme.background, minHeight: "100vh" }}>
      <div style={pageWrapStyle()}>
        ...
        {cargando ? (
          ...
          <Card style={{ textAlign: "center", padding: "30px" }}><p style={{ color: theme.textMuted }}>No tenés licitaciones abiertas por ahora.</p></Card>
        ) : (
                ...
                <Card key={r.id} style={{ marginBottom: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "12px" }}>
                    <div style={{ flex: 1, minWidth: "220px" }}>
                      ...
                      {r.especificaciones_tecnicas && <div style={{ color: "#888", fontSize: "0.75rem", marginTop: "4px" }}>Specs: {r.especificaciones_tecnicas}</div>}
                      <div style={{ color: "#888", fontSize: "0.75rem", marginTop: "6px" }}>
                        Cantidad: {r.cantidad} {r.unidad || "und"} · Cierre: {new Date(r.fecha_limite_ofertas).toLocaleString()}
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <Badge tone={r.estado === "Adjudicada" ? (r.rfq_ganador_proveedor_id === proveedor.id ? "success" : "neutral") : puedeOfertar ? "gold" : "neutral"}>
                        {r.estado === "Adjudicada" ? (r.rfq_ganador_proveedor_id === proveedor.id ? "¡Ganaste!" : "Adjudicada a otro proveedor") : r.estado}
                      </Badge>
                      <div style={{ marginTop: "10px" }}>
                        {puedeOfertar ? (
                          <Button variant="gold" style={{ padding: "6px 14px", fontSize: "0.78rem" }} onClick={() => abrirOferta(r)}>
                            {yaOferte ? "Editar mi oferta" : "Enviar oferta"}
                          </Button>
                        ) : yaOferte ? (
                          <span style={{ color: "#888", fontSize: "0.75rem" }}>Ya enviaste tu oferta</span>
                        ) : (
                          <span style={{ color: "#888", fontSize: "0.75rem" }}>Cerrada</span>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ background: theme.panelBg, border: `1px solid ${theme.borderGoldCounter}`, borderRadius: theme.radiusLg, padding: "26px", width: "100%", maxWidth: "480px" }}>
            ...
            <p style={{ color: "#888", fontSize: "0.75rem", marginBottom: "16px" }}>
              Tu oferta es confidencial: ni otros proveedores ni nadie más pueden verla hasta que cierre la licitación.
            </p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
              ...
              <div><label style={labelStyle}>Lead time (días)</label>
              ...
              <div style={{ gridColumn: "1 / -1" }}><label style={labelStyle}>Días de crédito que ofrecés</label>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
              ...
              <Button variant="gold" disabled={guardando} onClick={enviarOferta}>{guardando ? "Enviando..." : "Enviar oferta"}</Button>
            </div>
          </div>
        </div>
      )}
